...
const Projects = () => {
  ...
  const fetchProjects = async () => {
    try {
      const { data } = await axios.get(
        `https://zzmrjhftlghyxczgaqri.supabase.co/storage/v1/object/public/project_lists/projects.json?t=${new Date().getTime()}`,
      );

      if (data) {
        const baseUrl =
          "https://zzmrjhftlghyxczgaqri.supabase.co/storage/v1/object/public/portfolio_images/";
        const files = await listAllFiles("portfolio_images");

        const urls = files.map((file) => {
          return {
            project: file.name.split("/")[0],
            url: baseUrl + file.name,
          };
        });
        const newProjects = data.map((project) => {
          const images = urls
            .filter((url) => url.project == project.bucketname)
            .map((img) => img.url);
          return {
            ...project,
  ...
  const RenderProjectImages = React.memo(({ item, imgsrc }) => {
    return (
      <div className="relative flex-1 flex overflow-hidden groups rounded-2xl aspect-[1920/1080] border-2 border-gray-800">
        {/* Image Background */}
        <div className="w-full bg-center bg-inherit bg-no-repeat h-full">
          <img src={imgsrc} className="w-full h-full object-fit" />
        </div>

        {/* Action Buttons */}
        <div
          className={`absolute bg-black/70 w-full h-full ${hoveredProject === item.id ? "flex" : "hidden"} flex-col items-center justify-center gap-3 transition-all duration-300 
        transform`}
        >
          {item.video && (
            <button
    ...
    >
            ...
            `}
              >
                ...
                <div className="relative h-full overflow-hidden transition-all duration-500 border shadow-2xl bg-themeDark backdrop-blur-xl border-gray-700/50 rounded-3xl hover:shadow-3xl">
                  {/* Top Accent Line */}
                  <div
                    className={`${hoveredProject === item.id ? "h-1 w-full" : "h-0 w-0"} bg-gradient-to-r ${item.color} transform transition-all duration-500 `}
                  />
                  ...
                  <div className="px-6 py-3 flex flex-col justify-between h-[250px] md:h-[220px] xl:h-[230px]">
                    ...
                    <>
                      <div>
                        ...
                        <p
                          className={`text-sm font-medium text-themeSecondary`}
                        >
                          {item.subtitle}
                        </p>
                      </div>
      ...
      >
        <div className="relative w-full max-w-4xl overflow-hidden bg-black shadow-2xl rounded-2xl">
          ...
          <video width="100%" controls autoPlay className="rounded-2xl">
